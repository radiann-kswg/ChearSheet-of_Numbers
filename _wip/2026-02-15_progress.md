...
- 数式（KaTeX）運用を前提に、VS Code / 指示書 / 生成を整備
  ...
  - Wikipedia「性質」セクション由来の短い引用を追加し、引用を崩さずに別行で「数式（整形）」を追記する仕組みを追加
    - 例: `π = 3.1415…` などを `$\pi = 3.1415\dots$` に整形
  - Wikipedia「性質」セクションの取得はキャッシュ優先（`--refresh-wikipedia-sections` を指定した場合のみ追加取得）
