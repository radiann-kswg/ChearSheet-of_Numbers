...
- 0〜999 の再生成を実施（内容が同一の場合は差分が出ないことがある）
  - 400–499 のオンライン生成でネットワークエラー（DNS解決失敗）に遭遇したため、`--offline`（キャッシュのみ）で 400–999 を再開して完走
- `index.md` → `numbers/**` の内部リンク整合を機械チェックし、リンク切れが無いことを確認（相対リンク 7699 件 / Missing 0）

## 追加対応（同日・続き2: refresh運用の安定化）

- refresh 付き全体再生成の運用中に、生成プロセスの重複/残留や「実行したはずのレンジが更新されていないように見える」事象が出たため、直列実行・mtime確認・小範囲検証で切り分け
- API 疎通チェックを短時間に繰り返すと 429（Too many requests）を引き起こし得ることを確認
  - 対応: Wikipedia/Wikidata 側の HTTP 取得（`_http_get_json`）に 429 を考慮した待機（`Retry-After` 優先、無ければ最小30秒）を追加し、バックオフを強化
- キャッシュ refresh と生成の手順を分離し、ネットワーク状態に左右されにくい流れを整備
  - オンラインでキャッシュ refresh → その後は `--offline` で全ページを再生成（キャッシュ内容を確実に反映）
  - 内部リンク検査は最終段で再実行し Missing 0 を維持
- 補助スクリプト（ハートビート付きで長時間処理を追跡）を追加
  - `tools/full_refresh_generate.py`: refresh + 全体再生成 + 内部リンク検査
  - `tools/offline_generate_all.py`: オフライン全体再生成 + 内部リンク検査
  - `tools/refresh_and_generate_all.py`: レンジ分割の refresh/生成（主に切り分け用）
  - `tools/refresh_and_generate_all.ps1`: PowerShell 用（実行ポリシーでブロックされる環境があるため注意）

## 追加対応（同日・続き3: 引用エッセンスの件数仕様）

- Wikipedia『性質』『その他』の要点（短い引用）は、従来「最大3件」相当の挙動だったが、
  **重要度（閾値）を満たす要点が4件以上ある場合は全件記載**する仕様へ変更
  - 3件は「最低限の表示件数」として維持し、閾値未満のときのみ上位候補で補完

## 追加対応（同日・続き4: 新旧アルゴリズム結果の併記）

- Wikipedia『性質』『その他』の要点について、
  - 現行アルゴリズム（閾値以上は全件）
  - 旧アルゴリズム（最大3件）
    の **両方の結果をページに併記**できるようにした
  - 重複（同一エッセンス）は 1 回だけ出し、**ラベル無しでマージして出力**
  - キャッシュ形式を拡張し、`tools/_cache/wikipedia_ja_properties_v1.json` に `properties_legacy`、`tools/_cache/wikipedia_ja_others_v1.json` に `others_legacy` を追加
...
- `tools/full_refresh_generate.py`
- `tools/offline_generate_all.py`
- `tools/refresh_and_generate_all.py`
- `tools/refresh_and_generate_all.ps1`
