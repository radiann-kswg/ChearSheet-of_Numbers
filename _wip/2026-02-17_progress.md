...
## 追記（pins/閾値調整の反映・全件 refresh 再生成）

### 背景

- 「除外してほしくなかった Wikipedia 引用」リストに基づき pins と閾値 overrides を追加し、特定の数字で確実に採用されるようにした。
- 918 などで pins を入れても「その他」の引用が出ないケースがあり、抽出・選抜・キャッシュの再生成を含めた運用で整合を取る必要があった。

### 変更点の要約

- pins/overrides の調整
  - `tools/wikipedia_ja_pins_v1.json` を拡充（対象番号の「拾いたいフレーズ」を部分一致 pins として追加）。
  - `tools/wikipedia_ja_importance_overrides_v1.json` に `other: 20` を追加し、pins 以外の候補も拾いやすくした。
- 抽出側の品質
  - `tools/wikipedia_ja.py` で「その他 N に関連すること。」のような内容ゼロ stub を、他に有意な候補がある場合は最終結果から落とすように調整。
  - これにより fallback 補完で stub が再混入するケースを抑制。
- pins の追随
  - 747 の pins は Wikipedia 側文面と乖離（`747-400` を含まない）していたため、いったん `ボーイング747` に寄せる形へ調整。

### 動作確認

- 全件（0..999）を `--wikipedia-sections --refresh-wikipedia-sections` で再生成（セクションキャッシュも更新）。
- 内部リンクチェック: `tools/check_internal_links.py`（Scanned markdown files: 1007 / Missing links: 0）

### 影響範囲（主）

- tools/wikipedia_ja.py
- tools/wikipedia_ja_pins_v1.json
- numbers/ 配下（全数再生成）
